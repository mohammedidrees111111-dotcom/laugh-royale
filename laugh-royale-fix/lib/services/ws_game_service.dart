// laugh-royale/lib/services/ws_game_service.dart
// FIXED VERSION - With laugh event support
...
class WsGameService {
  ...
  static Future _attemptReconnect() async {
  ...
  static void _onMessage(dynamic data) {
    try {
      ...
      // FIXED: Handle laugh event from opponent
      if (type == 'laugh_event') {
        final timestamp = msg['timestamp'] as int?;
        debugPrint('[WS] Opponent laughed at timestamp: $timestamp');
        _controller.add({
          'type': 'event',
          'event': 'laughed',
          'timestamp': timestamp,
  ...
  // FIXED: Send laugh event with timestamp
  static void sendLaughEvent(int timestamp) {
    final msg = {
      'type': 'laugh_event',
      'timestamp': timestamp,
    ...
    if (_ws != null && _ws!.readyState == WebSocket.open) {
      _send(msg);
      debugPrint('[WS] Sent laugh event with timestamp: $timestamp');
    } else {
      debugPrint('[WS] Socket not open — buffering laugh event');
