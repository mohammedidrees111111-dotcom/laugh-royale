// laugh-royale/server/server.js
// FIXED VERSION - With laugh event timestamp comparison
...
function tryPair() {
  while (matchmakingQueue.length >= 2) {
    ...
    c2.role = 'guest';
    // FIXED: Initialize laugh tracking for the room
    rooms.set(roomCode, {
      host: p1,
      guest: p2,
      hostId: c1.playerId,
...
// FIXED: Handle laugh event and determine winner
function handleLaughEvent(ws, msg) {
  const cs = connStateMap.get(ws);
  if (!cs || !cs.roomCode) {
  ...
  if (room.gameOver) {
    log('LAUGH', `Room ${cs.roomCode} already has winner - ignoring late laugh`);
    return;
  }
  ...
  log('LAUGH', `${cs.role}[${cs.playerId}] laughed at ${timestamp}`);

  if (isHost) {
    if (room.hostLaughed) {
      log('LAUGH', 'Host already laughed - ignoring duplicate');
      return;
    }
    room.hostLaughed = true;
    room.hostLaughTime = timestamp;
  } else {
    if (room.guestLaughed) {
      log('LAUGH', 'Guest already laughed - ignoring duplicate');
      return;
    }
    room.guestLaughed = true;
    room.guestLaughTime = timestamp;
  }

  // Relay laugh event to opponent
  const opponent = getOpponent(ws, room);
  if (opponent && opponent.readyState === WebSocket.OPEN) {
    ...
    log('LAUGH', `Relayed laugh event from ${cs.role} to opponent`);
  }

  // FIXED: Determine winner if both laughed
  if (room.hostLaughed && room.guestLaughed) {
    room.gameOver = true;
    const hostWon = room.hostLaughTime <= room.guestLaughTime;

    log('LAUGH', `Both laughed! hostLaughTime=${room.hostLaughTime}, guestLaughTime=${room.guestLaughTime}`);
    log('LAUGH', `Winner: ${hostWon ? 'HOST' : 'GUEST'} (who laughed first or same time)`);
    ...
    if (room.host && room.host.readyState === WebSocket.OPEN) {
      send(room.host, {
        ...
        reason: 'opponent_laughed_first',
      });
    }
    if (room.guest && room.guest.readyState === WebSocket.OPEN) {
      send(room.guest, {
        type: 'game_result',
        winner: hostWon ? 'guest' : 'host',
        reason: 'you_laughed_first',
      });
    }
  }
}
    ...
    switch (msg.type) {
      ...
      case 'host':
        ...
        rooms.set(cs.roomCode, {
          ...
          hostLaughed: false, guestLaughed: false,
          hostLaughTime: null, guestLaughTime: null,
          gameOver: false,
        });
      ...
      // FIXED: Handle laugh event with timestamp
      case 'laugh_event':
        handleLaughEvent(ws, msg);
        break;
